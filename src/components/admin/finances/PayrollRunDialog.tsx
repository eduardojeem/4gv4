  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Preparar nómina</DialogTitle>
          <DialogDescription>
            Revisa los importes devengados por el servidor antes de crear una corrida. La aprobación se confirma desde la corrida creada.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4">
          ...
          <Button type="button" variant="outline" onClick={loadPreview} disabled={isLoadingPreview}>
            {isLoadingPreview ? 'Calculando…' : 'Ver vista previa'}
          </Button>
          {preview ? (
            <section aria-live="polite" className="rounded-md border p-3">
              <p className="font-medium">Total neto: {formatCurrency(preview.totals.netPay)}</p>
              <ul className="mt-2 divide-y">
                {preview.entries.map((entry) => (
                  <li key={entry.employeeId} className="flex justify-between gap-3 py-2 text-sm">
                    <span>{entry.employeeId} · {entry.role}</span>
          ...
          <p className="text-sm text-muted-foreground">
            Las excepciones individuales del empleado prevalecen sobre las reglas de rol.
          </p>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={createRun} disabled={!preview || isSubmitting}>
