...
export function PaymentDialog({
  ...
  async function submit(formData: FormData) {
    ...
    // Client-side guard: reject amounts exceeding outstanding balance
    if (outstandingAmount !== undefined && amount > outstandingAmount) {
      setError(`El monto no puede superar el pendiente autorizado (${outstandingAmount.toLocaleString('es-PY')}).`)
      return
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>El servidor valida el saldo disponible y la sesión de caja.</DialogDescription>
        </DialogHeader>
        <form action={submit} className="grid gap-4">
          {outstandingAmount !== undefined ? (
            <p className="text-sm text-muted-foreground">
              Pendiente autorizado: ₲ {Math.round(outstandingAmount).toLocaleString('es-PY')}
            </p>
          ) : null}
          ...
          <label className="grid gap-1 text-sm font-medium">
            Referencia (opcional)
            <input name="reference" className="rounded-md border bg-background px-3 py-2" />
          </label>
