...
type ReportTab = 'sales' | 'products' | 'categories' | 'customers' | 'repairs' | 'credits'
type ReportPeriod = '7d' | '30d' | '90d' | '1y'

const REPORT_TABS = new Set<ReportTab>(['sales', 'products', 'categories', 'customers', 'repairs', 'credits'])
const REPORT_PERIOD_DAYS: Record<ReportPeriod, number> = { '7d': 7, '30d': 30, '90d': 90, '1y': 365 }

function isReportTab(value: string | null): value is ReportTab {
  return value !== null && REPORT_TABS.has(value as ReportTab)
}

function isReportPeriod(value: string | null): value is ReportPeriod {
  return value !== null && value in REPORT_PERIOD_DAYS
}

function resolveAvailableTab(value: string | null, hasRepairs: boolean, hasCredits: boolean): ReportTab {
  if (!isReportTab(value)) return 'sales'
  if (value === 'repairs' && !hasRepairs) return 'sales'
  if (value === 'credits' && !hasCredits) return 'sales'
  return value
}
...
export default function OperationalReports() {
  ...
  const initialPeriod: ReportPeriod = isReportPeriod(requestedPeriod) ? requestedPeriod : '30d'
  const [activeTab, setActiveTab] = useState<ReportTab>(() => resolveAvailableTab(requestedTab, hasRepairs, hasCredits))
  const [dateRange, setDateRange] = useState<{ from: Date; to: Date }>(() => createCalendarPeriodRange(new Date(), REPORT_PERIOD_DAYS[initialPeriod]))
  const [selectedPeriod, setSelectedPeriod] = useState<ReportPeriod>(initialPeriod)
  const needsProductData = activeTab === 'products' || activeTab === 'categories'
  const needsRepairData = activeTab === 'repairs'

  const updateReportUrl = useCallback((nextTab: ReportTab, nextPeriod: ReportPeriod) => {
  ...
  const handleTabChange = useCallback((value: string) => {
    ...
    const nextTab = resolveAvailableTab(value, hasRepairs, hasCredits)
    setActiveTab(nextTab)
    updateReportUrl(nextTab, selectedPeriod)
  }, [hasCredits, hasRepairs, selectedPeriod, updateReportUrl])

  const handlePeriodChange = useCallback((value: ReportPeriod) => {
    setSelectedPeriod(value)
    updateReportUrl(activeTab, value)
  }, [activeTab, updateReportUrl])

  useEffect(() => {
    const nextPeriod = isReportPeriod(requestedPeriod) ? requestedPeriod : '30d'
    const nextTab = resolveAvailableTab(requestedTab, hasRepairs, hasCredits)

    setActiveTab((current) => current === nextTab ? current : nextTab)
    setSelectedPeriod((current) => current === nextPeriod ? current : nextPeriod)
  }, [hasCredits, hasRepairs, requestedPeriod, requestedTab])
  ...
  useEffect(() => {
    const fetchReportsData = async () => {
      ...
      try {
        ...
        let itemSales: Array<{ id: string; created_at: string; status: string | null }> = []
        let itemSalesError: { message: string } | null = null
        if (needsProductData) {
          const itemSalesResult = await withBranchFilter(
            supabase
              .from('sales')
          ...
          itemSales = itemSalesResult.data ?? []
          itemSalesError = itemSalesResult.error
        }
        ...
        const { data: repairsData, error: repairsError } = needsRepairData ? await withBranchFilter(
          supabase
            .from('repairs')
        ...
        // en que se lo bajaba.
        const { data: deliveredData, error: deliveredError } = needsRepairData ? await withBranchFilter(
          supabase
            .from('repairs')
    ...
    fetchReportsData()
  }, [canViewCost, categoryDateRange, dateRange, needsProductData, needsRepairData, organization?.id, refreshTrigger, selectedBranchId, toLocalDateKey])

  useEffect(() => {
    if (activeTab !== 'credits' || !hasCredits || !organization?.id) {
  ...
  }, [activeTab, dateRange, hasCredits, organization?.id, refreshTrigger, selectedBranchId])
  ...
  useEffect(() => {
    ...
    const controller = new AbortController()
    const fetchCustomerAccessReport = async () => {
      setCustomerAccessLoading(true)
      setCustomerAccessError(null)
      try {
        const params = new URLSearchParams({
          from: dateRange.from.toISOString(),
          to: dateRange.to.toISOString(),
        ...
        const body = await response.json().catch(() => null) as {
          success?: boolean
          data?: CustomerAccessReport
          error?: string
        } | null
        if (!response.ok || !body?.success || !body.data) {
          throw new Error(body?.error || 'No se pudo cargar el acceso de clientes.')
        }
        setCustomerAccessReport(body.data)
      } catch (error) {
        if (controller.signal.aborted) return
        setCustomerAccessError(error instanceof Error ? error.message : 'No se pudo cargar el acceso de clientes.')
      } finally {
        if (!controller.signal.aborted) setCustomerAccessLoading(false)
      }
    }
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-3 py-5 sm:px-6 sm:py-8 lg:px-8">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between pb-3 border-b border-border/40">
        ...
        <div className="flex w-full flex-wrap items-center gap-2 lg:w-auto lg:justify-end">
          ...
          <div className="flex max-w-full flex-1 items-center overflow-x-auto rounded-xl border border-slate-200/80 bg-slate-100/90 p-1 shadow-2xs sm:flex-none dark:border-slate-800 dark:bg-slate-900/90">
            {[
              { id: '7d', label: '7 Días' },
              ...
              <button
                key={p.id}
                type="button"
                onClick={() => handlePeriodChange(p.id as ReportPeriod)}
                aria-pressed={selectedPeriod === p.id}
                className={`min-h-9 flex-1 whitespace-nowrap rounded-lg px-3 py-1 text-xs font-semibold transition-all cursor-pointer sm:flex-none ${
                  selectedPeriod === p.id
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-5">
        <div className="sticky top-2 z-20 -mx-1 overflow-x-auto rounded-xl border border-slate-200/80 bg-white/95 p-1.5 shadow-sm backdrop-blur sm:static sm:mx-0 sm:rounded-2xl dark:border-slate-800/80 dark:bg-slate-950/95">
          <TabsList aria-label="Secciones de reportes" className="inline-flex h-auto min-w-max gap-1 bg-transparent p-0">
            <TabsTrigger
              value="sales"
