...
const statusConfig: Partial<Record<RepairStatus, { label: string, color: string, priority: number }>> = {
  recibido: {
    label: 'Recibido',
...
function getDaysSince(createdAt: string, currentTime: number) {
  ...
  return (currentTime - createdAtTime) / (1000 * 60 * 60 * 24)
}
...
export const OptimizedTechnicianActiveJobs = memo(function OptimizedTechnicianActiveJobs({
  ...
  const filteredAndSortedRepairs = useMemo(() => {
    ...
    const filteredRepairs = repairs.filter((repair) => {
      const matchesSearch = !searchTerm ||
        repair.customer.name.toLowerCase().includes(normalizedSearch) ||
        repair.device.toLowerCase().includes(normalizedSearch) ||
        repair.issue.toLowerCase().includes(normalizedSearch) ||
        repair.id.toLowerCase().includes(normalizedSearch)

      const matchesStatus = statusFilter === 'all' || resolveRepairStatus(repair) === statusFilter
    ...
    return [...filteredRepairs].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'status': {
          const aStatus = statusConfig[resolveRepairStatus(a)]?.priority || 999
          const bStatus = statusConfig[resolveRepairStatus(b)]?.priority || 999
  ...
  }, [priorityFilter, repairs, searchTerm, sortBy, statusFilter])

  const stats = useMemo(() => {
    const urgent = repairs.filter((repair) => repair.urgency === 'urgent').length
    const highPriority = repairs.filter((repair) => repair.priority === 'high').length
    const overdue = repairs.filter((repair) => getDaysSince(repair.createdAt, currentTime) > 7).length
      ...
      <CardContent>
        <div className="space-y-3">
          {filteredAndSortedRepairs.map((repair) => {
            ...
            const daysSince = getDaysSince(repair.createdAt, currentTime)
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <div className="text-right">
                        <div className="font-medium">
                          {Math.round(daysSince)} dia{Math.round(daysSince) !== 1 ? 's' : ''}
                        </div>
                      </div>
                    </div>
                  </div>
