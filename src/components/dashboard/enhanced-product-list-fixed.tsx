...
const getMarginInfo = (product: Product) => {
  ...
  if (marginPercentage < 10) {
    ...
    return { margin, marginPercentage, color: 'text-yellow-600', bgColor: 'bg-yellow-100', label: 'Margen medio' }
  } else {
    return { margin, marginPercentage, color: 'text-green-600', bgColor: 'bg-green-100', label: 'Margen alto' }
  }
}
...
const MarginIndicator = ({ product }: { product: Product }) => {
  const marginInfo = getMarginInfo(product)

  return (
    <div className="space-y-1">
      <div className="flex items-center space-x-2">
        <Badge variant="outline" className={cn("text-xs", marginInfo.color, marginInfo.bgColor)}>
          {marginInfo.marginPercentage.toFixed(1)}%
        </Badge>
      </div>
            ...
            <TableRow>
              <TableHead className="w-12">
                <input
                  type="checkbox"
                  checked={allSelected}
                  onChange={(e) => onProductSelectAll?.(e.target.checked)}
                  className="h-4 w-4"
                />
              </TableHead>
            ...
            {products.map((product) => {
              ...
              return (
                <TableRow key={product.id} className={isSelected ? "bg-muted/50" : ""}>
                  ...
                  <TableCell>
                    <MarginIndicator product={product} />
                  </TableCell>
                  <TableCell>
