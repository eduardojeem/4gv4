...
import { AlertTriangle, ArrowLeft, Check, CircleDollarSign, Loader2, Package, Tags, Wrench } from 'lucide-react'
...
function RepairCostsEditorForm({ open, repair, maxDiscountPercent, laborTaxRate, onOpenChange, onSaved }: {
  ...
  const input = useMemo(() => ({ currency: 'PYG', laborAmount, laborTaxRate,
    parts: parts.map((part) => ({ key: part.key, quantity: part.quantity, unitPrice: part.unitPrice, unitCost: part.unitCost, discountAmount: part.discountAmount, taxRate: part.taxRate, lineType: part.lineType })),
    ...
    <DialogContent className="flex h-[96dvh] max-h-[96dvh] w-[calc(100%-1rem)] max-w-[1400px] flex-col gap-0 overflow-hidden p-0 sm:max-w-[1400px] lg:w-[calc(100%-2rem)]">
      ...
      <div className="flex-1 overflow-y-auto bg-muted/20 p-3 sm:p-5">
        {step === 'edit' ? <div className="grid gap-5 lg:grid-cols-[minmax(0,2.2fr)_minmax(340px,0.8fr)]"><div className="min-w-0 space-y-4">
          {manualServiceOpen && <section className="rounded-xl border border-primary/30 bg-primary/5 p-4"><div><h3 className="font-semibold">Agregar servicio manual</h3><p className="text-xs text-muted-foreground">Se mostrará como servicio cobrado y no modificará el inventario.</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div><Label htmlFor="manual-service-name">Nombre del servicio</Label><Input id="manual-service-name" value={manualServiceName} onChange={(event) => setManualServiceName(event.target.value)} className="mt-1" /></div><div><Label htmlFor="manual-service-amount">Precio al cliente</Label><MoneyInput id="manual-service-amount" value={manualServiceAmount} onChange={setManualServiceAmount} /></div></div><div className="mt-4 flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => setManualServiceOpen(false)}>Cancelar servicio</Button><Button type="button" disabled={manualServiceAmount <= 0 || !manualServiceName.trim()} onClick={() => { setParts((current) => [...current, { key: `service-${crypto.randomUUID()}`, productId: null, name: manualServiceName.trim(), supplier: 'Carga manual', quantity: 1, unitPrice: manualServiceAmount, unitCost: 0, discountAmount: 0, taxRate: laborTaxRate, availableStock: null, lineType: 'service' }]); setManualServiceOpen(false); setManualServiceAmount(0) }}>Agregar servicio</Button></div></section>}
          <section className="rounded-xl border bg-card p-4"><div className="mb-4 flex items-center gap-2"><Wrench className="h-4 w-4 text-muted-foreground" /><div><h3 className="font-semibold">Mano de obra adicional</h3><p className="text-xs text-muted-foreground">{costKind === 'service' ? 'El servicio ya conserva su precio completo. Usá este campo solo si existe una mano de obra adicional explícita.' : costKind === 'parts' ? 'Esta reparación no tiene mano de obra cargada. Podés dejarla en cero.' : 'Todavía no se cargaron costos para esta reparación.'}</p></div></div><Label htmlFor="fixed-labor">Mano de obra adicional opcional</Label><MoneyInput id="fixed-labor" value={laborAmount} onChange={setLaborAmount} prominent /></section>
          <section className="rounded-xl border bg-card p-4"><div className="mb-4"><h3 className="font-semibold">Servicios, repuestos y materiales</h3><p className="text-xs text-muted-foreground">Los servicios se cobran una vez; los materiales incluidos solo forman parte del costo interno.</p></div><RepairPartsEditor parts={parts} onChange={setParts} onAddService={() => { setManualServiceAmount(0); setManualServiceOpen(true) }} repairId={repair.id} customerIsWholesale={customerIsWholesale} disabled={saving} invalidPartKeys={invalidPartKeys} /></section>
...
function CostKindBadge({ kind }: { kind: 'service' | 'parts' | 'pending' }) { const label = kind === 'service' ? 'Servicio técnico' : kind === 'parts' ? 'Solo repuestos' : 'Costos pendientes'; const Icon = kind === 'service' ? Wrench : Package; return <span className="inline-flex items-center gap-1 rounded-full border bg-muted/60 px-2.5 py-1 text-xs font-medium"><Icon className="h-3.5 w-3.5" />{label}</span> }
...
function Preview({ summary, parts }: { summary: ReturnType<typeof calculateRepairCost>; parts: EditableRepairPart[] }) {
  const groups = [
  ...
  return <div className="mx-auto max-w-3xl space-y-4">
    <section className="rounded-xl border bg-card p-4">
      <h3 className="font-semibold">Composición del monto</h3>
      {summary.laborAmount > 0 && <div className="mt-3 flex justify-between rounded-lg bg-muted/40 px-3 py-2 text-sm"><span>Mano de obra adicional</span><strong>{formatCurrency(summary.laborAmount)}</strong></div>}
      {groups.map((group) => {
        const rows = parts.filter((part) => (part.lineType ?? 'charged_part') === group.type)
        if (rows.length === 0) return null
        return <div key={group.type} className="mt-4 overflow-hidden rounded-lg border">
          <h4 className="bg-muted/50 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{group.label}</h4>
          <dl className="divide-y text-sm">{rows.map((part) => <div key={part.key} className="flex justify-between gap-4 px-3 py-2"><dt>{part.quantity} × {part.name}{group.type === 'included_material' && <small className="block text-muted-foreground">Incluido · Gs. 0 adicional al cliente</small>}</dt><dd className="font-medium tabular-nums">{formatCurrency(group.type === 'included_material' ? 0 : Math.max(0, part.quantity * part.unitPrice - part.discountAmount))}</dd></div>)}</dl>
        </div>
      })}
      {(summary.discountAmount + summary.deductions) > 0 && <div className="mt-3 flex justify-between py-2 text-sm text-rose-600"><span>Descuentos y deducciones</span><strong>- {formatCurrency(summary.discountAmount + summary.deductions)}</strong></div>}
    </section>
    <section className="rounded-xl border bg-card p-4"><h3 className="font-semibold">IVA incluido</h3>{summary.taxBreakdown.map((tax) => <p key={tax.rate} className="mt-2 flex justify-between gap-4 text-sm"><span>Tasa {tax.rate}% · Base {formatCurrency(tax.taxableBase)}</span><strong className="tabular-nums">{formatCurrency(tax.taxAmount)}</strong></p>)}</section>
    <div className="rounded-xl border border-emerald-300 bg-emerald-50 p-5 dark:border-emerald-900 dark:bg-emerald-950/30"><div className="flex items-center gap-2 text-sm font-semibold"><CircleDollarSign className="h-5 w-5" />Monto total final</div><p className="mt-1 text-3xl font-bold text-emerald-700 tabular-nums dark:text-emerald-300">{formatCurrency(summary.finalTotal)}</p><p className="mt-3 border-t border-emerald-200 pt-3 text-sm dark:border-emerald-900">Pagado {formatCurrency(summary.paidAmount)} · Pendiente {formatCurrency(summary.balance)}</p></div>
  </div>
}
