...
function RepairCostsEditorForm({ open, repair, maxDiscountPercent, laborTaxRate, onOpenChange, onSaved }: {
  ...
  const costKind = laborAmount > 0 ? 'service' : parts.length > 0 ? 'parts' : 'pending'
    ...
    <DialogContent className="flex h-[96dvh] max-h-[96dvh] w-[calc(100%-1rem)] max-w-[1400px] flex-col gap-0 overflow-hidden p-0 sm:max-w-[1400px] lg:w-[calc(100%-2rem)]">
      ...
      <div className="flex-1 overflow-y-auto bg-muted/20 p-3 sm:p-5">
        {step === 'edit' ? <div className="grid gap-5 lg:grid-cols-[minmax(0,2.2fr)_minmax(340px,0.8fr)]"><div className="min-w-0 space-y-4">
          {manualServiceOpen && <section className="rounded-xl border border-primary/30 bg-primary/5 p-4"><div><h3 className="font-semibold">Agregar servicio manual</h3><p className="text-xs text-muted-foreground">Se incorporará como mano de obra y no modificará el inventario.</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div><Label htmlFor="manual-service-name">Nombre del servicio</Label><Input id="manual-service-name" value={manualServiceName} onChange={(event) => setManualServiceName(event.target.value)} className="mt-1" /></div><div><Label htmlFor="manual-service-amount">Monto del servicio</Label><MoneyInput id="manual-service-amount" value={manualServiceAmount} onChange={setManualServiceAmount} /></div></div><div className="mt-4 flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => setManualServiceOpen(false)}>Cancelar servicio</Button><Button type="button" disabled={manualServiceAmount <= 0 || !manualServiceName.trim()} onClick={() => { setLaborAmount(manualServiceAmount); setManualServiceOpen(false) }}>Aplicar servicio</Button></div></section>}
          ...
          <section className="rounded-xl border bg-card p-4"><div className="mb-4"><h3 className="font-semibold">Repuestos utilizados</h3><p className="text-xs text-muted-foreground">Buscá en inventario y ajustá cantidad, precio o descuento.</p></div><RepairPartsEditor parts={parts} onChange={setParts} onAddService={() => { setManualServiceAmount(laborAmount); setManualServiceOpen(true) }} repairId={repair.id} customerIsWholesale={customerIsWholesale} disabled={saving} invalidPartKeys={invalidPartKeys} /></section>
