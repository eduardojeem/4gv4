  ...
  return (
    <div className="rounded-xl border border-border bg-card shadow-sm">
      <div className="border-b border-border px-6 py-4">
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">Actualizá tus datos de contacto y ubicación</p>
      </div>
      ...
      <form onSubmit={onSubmit} className="p-6">
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            />
            {errors.phone && <p id="phone-error" className="flex items-center gap-1 text-xs text-destructive"><AlertCircle className="h-3 w-3" aria-hidden="true" />{errors.phone}</p>}
          </div>

          {/* Email (readonly) */}
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="profile-email" className="flex items-center gap-1.5 text-sm font-medium text-muted-foreground">
              <Shield className="h-3.5 w-3.5" /> Correo electrónico
            </Label>
            <Input id="profile-email" value={email} disabled className="h-11 opacity-60" />
            <p className="text-xs text-muted-foreground">Por seguridad, el correo se administra desde tu cuenta de acceso.</p>
          </div>
