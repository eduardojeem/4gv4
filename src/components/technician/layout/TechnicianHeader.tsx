...
export function TechnicianHeader() {
  ...
  const technicianStats = useMemo(() => {
    if (!currentTechnicianId) return { total: 0, urgent: 0, active: 0 };
    const myRepairs = repairs.filter((r) => r.technician?.id === currentTechnicianId);
    const urgent = myRepairs.filter(
      (r) =>
        r.urgency === "urgent" &&
        r.dbStatus !== "listo" &&
        r.dbStatus !== "entregado",
    );
    const active = myRepairs.filter(
      (r) => r.dbStatus === "reparacion" || r.dbStatus === "diagnostico",
    );
  ...
  return (
    <header className="sticky top-0 z-10 flex items-center justify-between px-6 py-3 bg-white/90 dark:bg-gray-900/90 border-b border-gray-200 dark:border-gray-800 backdrop-blur-sm">
      {/* Left: Title & Stats */}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="gap-2">
              <div className="h-8 w-8 rounded-full bg-linear-to-br from-orange-500 to-red-500 flex items-center justify-center">
