...
export const metadata: Metadata = {
  title: 'Mis Creditos',
  description: 'Revisa tu historial de creditos y pagos pendientes.',
  robots: { index: false, follow: false },
}

export default async function OrganizationCreditsPage({
  params,
}: {
  ...
  if (!user) {
    const creditPath = `/${organizationSlug}/perfil/creditos`
      ...
      <main className="container max-w-4xl py-12 px-4 relative flex-1">
        <div className="mb-8">
          <Button asChild variant="ghost" size="sm" className="-ml-2 mb-4 group text-muted-foreground hover:text-foreground">
            <Link href={`/${organizationSlug}/perfil`}>
