...
import { CartItem, PaymentSplit, PaymentMethodOption, Product } from './types'
...
export default function POSPage() {
  ...
  const categories = useMemo(() => {
    return ['all', ...new Set(inventoryProducts.map(p => p.category))]
  }, [inventoryProducts])

  // Rango de precios dinámico
  const priceRangeLimits = useMemo(() => {
    const prices = inventoryProducts.map(p => p.price)
    return {
      min: Math.min(...prices),
      max: Math.max(...prices)
    }
  ...
  const addToCart = useCallback((product: Product) => {
    return measureCartOperation(() => {
      ...
      if (!checkAvailability(product.id, requestedQuantity)) {
        toast.error(`Stock insuficiente. Disponible: ${currentProduct.stock}`)
        return
      }
      ...
      setCart(prev => {
        ...
        if (existingItem) {
          ...
          const inferredWholesale = product.wholesalePrice
          return [...prev, {
            id: product.id,
            name: product.name,
            sku: product.sku,
  ...
  const updateQuantity = useCallback((id: string, quantity: number) => {
    ...
    if (!checkAvailability(id, quantity)) {
      ...
      toast.error(`Stock insuficiente. Disponible: ${currentProduct?.stock || 0}`)
      return
    }
        ...
        <div className={`flex-1 flex flex-col ${sidebarCollapsed ? 'lg:ml-0' : 'lg:ml-0'}`}>
          ...
          <div className="flex-1 flex overflow-hidden bg-muted/5">
            ...
            <div className="flex-1 p-3 sm:p-4 overflow-y-auto pb-24 md:pb-4" role="main" aria-label="Lista de productos">
              ...
              {paginatedProducts.length > virtualizationThreshold ? (
