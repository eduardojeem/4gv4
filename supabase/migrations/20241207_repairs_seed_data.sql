...
-- 
-- Si no existen, primero ejecuta los scripts de customers y profiles
-- =====================================================
...
BEGIN
  ...
  IF customers_count = 0 THEN
    RAISE WARNING '⚠ No hay clientes en la base de datos';
    RAISE WARNING 'Ejecuta primero el script de customers';
  END IF;
  
  IF technicians_count = 0 THEN
    RAISE WARNING '⚠ No hay técnicos en la base de datos';
    RAISE WARNING 'Ejecuta primero el script de profiles/technicians';
  END IF;
...
-- Obtener IDs de clientes y técnicos existentes
DO $$
DECLARE
  customer1_id UUID;
  customer2_id UUID;
...
BEGIN
  ...
  IF customer1_id IS NULL THEN
    RAISE EXCEPTION 'No hay clientes disponibles. Ejecuta primero el script de customers.';
  END IF;
  
  IF tech1_id IS NULL THEN
    RAISE WARNING 'No hay técnicos disponibles. Las reparaciones se crearán sin técnico asignado.';
  END IF;
  ...
  (repair1_id, tech1_id, 'Juan Técnico', 'Cliente reporta que el dispositivo cayó desde 1.5m de altura.', false),
  (repair1_id, tech1_id, 'Juan Técnico', 'Pantalla original de Apple, cotización enviada al cliente.', true);
  
  -- Notas para reparación 2
  INSERT INTO repair_notes (repair_id, author_id, author_name, note_text, is_internal) VALUES
  (repair2_id, tech1_id, 'Juan Técnico', 'Batería hinchada detectada, riesgo de explosión. Prioridad alta.', true),
  (repair2_id, tech1_id, 'Juan Técnico', 'Cliente notificado sobre el estado de la batería.', false);
  
  -- Notas para reparación 3
  INSERT INTO repair_notes (repair_id, author_id, author_name, note_text, is_internal) VALUES
  (repair3_id, tech2_id, 'María Técnico', 'Limpieza profunda completada. Temperaturas ahora normales.', false),
  (repair3_id, tech2_id, 'María Técnico', 'BIOS actualizado a última versión. Pruebas de estrés OK.', true);
  
  -- Notas para reparación 4
  INSERT INTO repair_notes (repair_id, author_id, author_name, note_text, is_internal) VALUES
  (repair4_id, tech2_id, 'María Técnico', 'Reparación completada. Dispositivo probado 24 horas sin problemas.', false);
  
  -- Notas para reparación 8
  INSERT INTO repair_notes (repair_id, author_id, author_name, note_text, is_internal) VALUES
  (repair8_id, tech3_id, 'Carlos Técnico', 'Limpieza ultrasónica en proceso. Placa madre en revisión.', true);
  ...
  INSERT INTO repair_parts (repair_id, part_name, part_number, quantity, unit_cost, supplier, status) VALUES
  (repair3_id, 'Pasta Térmica Arctic MX-4', 'MX-4-4G', 1, 150.00, 'Home & Garden Supplies', 'installed'),
  (repair3_id, 'Kit de limpieza para laptop', 'CLEAN-KIT-01', 1, 250.00, 'Home & Garden Supplies', 'installed');
...
-- Actualizar parts_cost en repairs basado en las partes insertadas
UPDATE repairs r
SET parts_cost = (
...
-- Actualizar final_cost donde no está establecido
UPDATE repairs
