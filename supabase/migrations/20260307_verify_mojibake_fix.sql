-- Verificacion post-fix de mojibake.
-- Seguro ante esquemas distintos de system_settings.

-- 1) Resumen de respaldos creados (si existe tabla backup)
CREATE TEMP TABLE IF NOT EXISTS tmp_backup_summary (
  table_name text,
  column_name text,
  fixed_rows bigint
) ON COMMIT DROP;

TRUNCATE TABLE tmp_backup_summary;

DO $$
BEGIN
  IF to_regclass('public.mojibake_fix_backup') IS NOT NULL THEN
    INSERT INTO tmp_backup_summary(table_name, column_name, fixed_rows)
    SELECT table_name, column_name, COUNT(*) AS fixed_rows
    FROM public.mojibake_fix_backup
    GROUP BY table_name, column_name;
  END IF;
END $$;

SELECT *
FROM tmp_backup_summary
ORDER BY fixed_rows DESC, table_name, column_name;

-- 2) Candidatos restantes por columnas comunes
CREATE TEMP TABLE IF NOT EXISTS tmp_mojibake_remaining (
  table_ref text,
  suspicious_rows bigint
) ON COMMIT DROP;

TRUNCATE TABLE tmp_mojibake_remaining;
...
BEGIN
  ...
  LOOP
    IF EXISTS (
      ...
      EXECUTE format($fmt$
        INSERT INTO tmp_mojibake_remaining(table_ref, suspicious_rows)
        SELECT %L, COUNT(*)
        FROM %I.%I
        WHERE %s;
...
SELECT *
FROM tmp_mojibake_remaining
WHERE suspicious_rows > 0
ORDER BY suspicious_rows DESC, table_ref;

-- 3) Revisar settings key/value pendientes (si columnas existen)
CREATE TEMP TABLE IF NOT EXISTS tmp_settings_pending (
  source text,
  row_ref text,
  raw_text text
) ON COMMIT DROP;

TRUNCATE TABLE tmp_settings_pending;
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format($fmt$
      INSERT INTO tmp_settings_pending(source, row_ref, raw_text)
